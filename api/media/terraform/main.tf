...
# Define REST API for Media
resource "aws_api_gateway_rest_api" "media_api" {
  name        = "MediaAPI"
  description = "API Gateway REST API for handling media uploads, retrieval, and deletion"
}

# CloudWatch Log Group for API Gateway
resource "aws_cloudwatch_log_group" "media_api_gw_logs" {
  name              = "/aws/api-gateway/media-api"
  retention_in_days = 14 # Customize as needed
}

# Deploy API Gateway for Media
resource "aws_api_gateway_deployment" "media_api_deployment" {
...
resource "aws_api_gateway_stage" "media_api_stage" {
  deployment_id = aws_api_gateway_deployment.media_api_deployment.id
  rest_api_id   = aws_api_gateway_rest_api.media_api.id
  stage_name    = "prod"

  access_log_settings {
    destination_arn = aws_cloudwatch_log_group.media_api_gw_logs.arn
    format = jsonencode({
  ...
  depends_on = [aws_api_gateway_deployment.media_api_deployment]
}

# Enable Detailed Logging & Metrics for All API Methods
resource "aws_api_gateway_method_settings" "all" {
  rest_api_id = aws_api_gateway_rest_api.media_api.id
  stage_name  = aws_api_gateway_stage.media_api_stage.stage_name
  method_path = "*/*"
...
resource "aws_api_gateway_base_path_mapping" "media_api_mapping" {
  api_id      = aws_api_gateway_rest_api.media_api.id
  stage_name  = aws_api_gateway_stage.media_api_stage.stage_name
